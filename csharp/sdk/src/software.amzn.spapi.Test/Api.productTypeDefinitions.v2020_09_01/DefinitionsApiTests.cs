...
namespace software.amzn.spapi.Test.Api.productTypeDefinitions.v2020_09_01
{
    ...
    public class DefinitionsApiTests
    {
        ...
        {
            Init();
            var url = "http://localhost:3000/response/" + ToLowerCaseAndCompress("definitions") + "-" + FormatOperationId("GetDefinitionsProductType") + "/code/200";
        ...
        {
            Init();
            var url = "http://localhost:3000/response/" + ToLowerCaseAndCompress("definitions") + "-" + FormatOperationId("SearchDefinitionsProductTypes") + "/code/200";
            var request = new HttpRequestMessage(HttpMethod.Post, url);
            httpClient.Send(request);
