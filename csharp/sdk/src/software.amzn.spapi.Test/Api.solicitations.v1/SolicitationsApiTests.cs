...
namespace software.amzn.spapi.Test.Api.solicitations.v1
{
    ...
    public class SolicitationsApiTests
    {
        ...
        {
            Init();
            var url = "http://localhost:3000/response/" + FormatOperationId("CreateProductReviewAndSellerFeedbackSolicitation") + "/code/201";
            var request = new HttpRequestMessage(HttpMethod.Post, AppendQualifier(url, "CreateProductReviewAndSellerFeedbackSolicitation"));
        ...
        {
            Init();
            var url = "http://localhost:3000/response/" + FormatOperationId("GetSolicitationActionsForOrder") + "/code/200";
            var request = new HttpRequestMessage(HttpMethod.Post, AppendQualifier(url, "GetSolicitationActionsForOrder"));
            httpClient.Send(request);
        ...
        private static string AppendQualifier(string url, string operationId) {
            if ("Api.solicitations.v1".Contains("vendor") && operationId.Equals("GetOrder")) url += "?qualifier=Vendor";
            if ("Api.solicitations.v1".Contains("fulfillment.inbound") && operationId.Equals("GetShipment")) url += "?qualifier=FbaInbound";
            if ("Api.solicitations.v1".Contains("sellerWallet") && operationId.Equals("GetAccount")) url += "?qualifier=SellerWallet";
            if ("Api.solicitations.v1".Contains("sellerWallet") && operationId.Equals("GetTransaction")) url += "?qualifier=SellerWallet";
            if ("Api.solicitations.v1".Contains("externalFulfillment") && operationId.Equals("GetShipment")) url += "?qualifier=ExternalFulfillment";
            if ("Api.solicitations.v1".Contains("externalFulfillment") && operationId.Equals("GetShipments")) url += "?qualifier=ExternalFulfillment";
            return url;
        }
    }
}
