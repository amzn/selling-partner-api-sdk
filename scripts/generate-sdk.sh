...
#
# The generator-cli version is pinned per language via an openapitools.json
# file committed in each language directory (e.g. java/openapitools.json).
# openapi-generator-cli resolves that file relative to its current working
# directory, so it MUST live alongside where this script is invoked from
# (see the "Run this from the language directory" note above) or the pin
# is silently ignored and the latest release is downloaded instead.
#
