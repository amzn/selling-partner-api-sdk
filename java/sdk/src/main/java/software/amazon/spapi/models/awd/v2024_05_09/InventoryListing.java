...
public class InventoryListing {
     ...
     * @throws IOException if the JSON Element is invalid with respect to InventoryListing
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!InventoryListing.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format(
                        "The required field(s) %s in InventoryListing is not found in the empty JSON string",
                        InventoryListing.openapiRequiredFields.toString()));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!InventoryListing.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format(
                        "The field `%s` in the JSON string is not defined in the `InventoryListing` properties. JSON: %s",
                        entry.getKey(), jsonElement.toString()));
            }
        }

        // check to make sure all required properties/fields are present in the JSON string
        for (String requiredField : InventoryListing.openapiRequiredFields) {
        ...
        for (int i = 0; i < jsonArrayinventory.size(); i++) {
            InventorySummary.validateJsonElement(jsonArrayinventory.get(i));
        }
        ;
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!InventoryListing.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'InventoryListing' and its subtypes
            }
            final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
            final TypeAdapter<InventoryListing> thisAdapter =
                    gson.getDelegateAdapter(this, TypeToken.get(InventoryListing.class));

            return (TypeAdapter<T>)
                    new TypeAdapter<InventoryListing>() {
                        @Override
                        public void write(JsonWriter out, InventoryListing value) throws IOException {
                            JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
                            elementAdapter.write(out, obj);
                        }

                        @Override
                        public InventoryListing read(JsonReader in) throws IOException {
    ...
    /**
     * Create an instance of InventoryListing given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of InventoryListing
     * @throws IOException if the JSON string is invalid with respect to InventoryListing
     */
    public static InventoryListing fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, InventoryListing.class);
    }

    /**
     * Convert an instance of InventoryListing to an JSON string
     *
     * @return JSON string
     */
