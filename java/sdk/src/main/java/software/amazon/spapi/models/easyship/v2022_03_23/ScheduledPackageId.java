...
public class ScheduledPackageId {
     ...
     * @throws IOException if the JSON Element is invalid with respect to ScheduledPackageId
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!ScheduledPackageId.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format(
                        "The required field(s) %s in ScheduledPackageId is not found in the empty JSON string",
                        ScheduledPackageId.openapiRequiredFields.toString()));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!ScheduledPackageId.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format(
                        "The field `%s` in the JSON string is not defined in the `ScheduledPackageId` properties. JSON: %s",
                        entry.getKey(), jsonElement.toString()));
            }
        }

        // check to make sure all required properties/fields are present in the JSON string
        for (String requiredField : ScheduledPackageId.openapiRequiredFields) {
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!ScheduledPackageId.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'ScheduledPackageId' and its subtypes
            }
            final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
            final TypeAdapter<ScheduledPackageId> thisAdapter =
                    gson.getDelegateAdapter(this, TypeToken.get(ScheduledPackageId.class));

            return (TypeAdapter<T>)
                    new TypeAdapter<ScheduledPackageId>() {
                        @Override
                        public void write(JsonWriter out, ScheduledPackageId value) throws IOException {
                            JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
                            elementAdapter.write(out, obj);
                        }

                        @Override
                        public ScheduledPackageId read(JsonReader in) throws IOException {
    ...
    /**
     * Create an instance of ScheduledPackageId given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of ScheduledPackageId
     * @throws IOException if the JSON string is invalid with respect to ScheduledPackageId
     */
    public static ScheduledPackageId fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, ScheduledPackageId.class);
    }

    /**
     * Convert an instance of ScheduledPackageId to an JSON string
     *
     * @return JSON string
     */
