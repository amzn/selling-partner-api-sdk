...
public class Preference {
    ...
    private Set<AutoEnrollmentPreference> autoEnrollment = new LinkedHashSet<>();

    public Preference() {}

    public Preference autoEnrollment(Set<AutoEnrollmentPreference> autoEnrollment) {
    ...
    public Preference addAutoEnrollmentItem(AutoEnrollmentPreference autoEnrollmentItem) {
        if (this.autoEnrollment == null) {
            this.autoEnrollment = new LinkedHashSet<>();
        }
     ...
     */
    @javax.annotation.Nullable public Set<AutoEnrollmentPreference> getAutoEnrollment() {
        return autoEnrollment;
    }

    public void setAutoEnrollment(Set<AutoEnrollmentPreference> autoEnrollment) {
        this.autoEnrollment = autoEnrollment;
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        if (jsonElement == null) {
            if (!Preference.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
                throw new IllegalArgumentException(String.format(
                        "The required field(s) %s in Preference is not found in the empty JSON string",
                        Preference.openapiRequiredFields.toString()));
            }
        }
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            if (!Preference.openapiFields.contains(entry.getKey())) {
                throw new IllegalArgumentException(String.format(
                        "The field `%s` in the JSON string is not defined in the `Preference` properties. JSON: %s",
        ...
        if (jsonObj.get("autoEnrollment") != null
                ...
                && !jsonObj.get("autoEnrollment").isJsonArray()) {
            throw new IllegalArgumentException(String.format(
                    "Expected the field `autoEnrollment` to be an array in the JSON string but got `%s`",
                    jsonObj.get("autoEnrollment").toString()));
        }
    }
    ...
    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            final TypeAdapter<Preference> thisAdapter = gson.getDelegateAdapter(this, TypeToken.get(Preference.class));

            return (TypeAdapter<T>)
                    new TypeAdapter<Preference>() {
                        @Override
