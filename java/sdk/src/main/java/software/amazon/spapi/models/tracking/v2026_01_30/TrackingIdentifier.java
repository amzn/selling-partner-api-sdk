/*
 * The Selling Partner API for tracking services provided by Amazon.
 * The Tracking Selling Partner API allows users to track shipments to and from Amazon-owned storage warehouses, fulfillment centers, and external warehouses.
 *
 * The version of the OpenAPI document: 2026-01-30
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

package software.amazon.spapi.models.tracking.v2026_01_30;

import com.google.gson.annotations.SerializedName;
import java.util.Objects;

/** The identifier to track a shipment. */
@io.swagger.v3.oas.annotations.media.Schema(description = "The identifier to track a shipment.")
public class TrackingIdentifier {
    @SerializedName("id")
    private String id = null;

    @SerializedName("carrierTracking")
    private CarrierTracking carrierTracking = null;

    @SerializedName("acsin")
    private String acsin = null;

    @SerializedName("aftn")
    private String aftn = null;

    @SerializedName("containerNumber")
    private String containerNumber = null;

    @SerializedName("houseBillOfLadingNumber")
    private String houseBillOfLadingNumber = null;

    public TrackingIdentifier id(String id) {
        this.id = id;
        return this;
    }

    /**
     * A unique identifier generated by Amazon for the shipment tracking request.
     *
     * @return id
     */
    @io.swagger.v3.oas.annotations.media.Schema(
            description = "A unique identifier generated by Amazon for the shipment tracking request.")
    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public TrackingIdentifier carrierTracking(CarrierTracking carrierTracking) {
        this.carrierTracking = carrierTracking;
        return this;
    }

    /**
     * Get carrierTracking
     *
     * @return carrierTracking
     */
    @io.swagger.v3.oas.annotations.media.Schema(description = "")
    public CarrierTracking getCarrierTracking() {
        return carrierTracking;
    }

    public void setCarrierTracking(CarrierTracking carrierTracking) {
        this.carrierTracking = carrierTracking;
    }

    public TrackingIdentifier acsin(String acsin) {
        this.acsin = acsin;
        return this;
    }

    /**
     * The Air Cargo Shipment Identification Number.
     *
     * @return acsin
     */
    @io.swagger.v3.oas.annotations.media.Schema(description = "The Air Cargo Shipment Identification Number.")
    public String getAcsin() {
        return acsin;
    }

    public void setAcsin(String acsin) {
        this.acsin = acsin;
    }

    public TrackingIdentifier aftn(String aftn) {
        this.aftn = aftn;
        return this;
    }

    /**
     * The Amazon Fulfillment Tracking Number.
     *
     * @return aftn
     */
    @io.swagger.v3.oas.annotations.media.Schema(description = "The Amazon Fulfillment Tracking Number.")
    public String getAftn() {
        return aftn;
    }

    public void setAftn(String aftn) {
        this.aftn = aftn;
    }

    public TrackingIdentifier containerNumber(String containerNumber) {
        this.containerNumber = containerNumber;
        return this;
    }

    /**
     * The container number provided by the Logistics Service Provider.
     *
     * @return containerNumber
     */
    @io.swagger.v3.oas.annotations.media.Schema(
            description = "The container number provided by the Logistics Service Provider.")
    public String getContainerNumber() {
        return containerNumber;
    }

    public void setContainerNumber(String containerNumber) {
        this.containerNumber = containerNumber;
    }

    public TrackingIdentifier houseBillOfLadingNumber(String houseBillOfLadingNumber) {
        this.houseBillOfLadingNumber = houseBillOfLadingNumber;
        return this;
    }

    /**
     * The House Bill of Lading (HBL) number.
     *
     * @return houseBillOfLadingNumber
     */
    @io.swagger.v3.oas.annotations.media.Schema(description = "The House Bill of Lading (HBL) number.")
    public String getHouseBillOfLadingNumber() {
        return houseBillOfLadingNumber;
    }

    public void setHouseBillOfLadingNumber(String houseBillOfLadingNumber) {
        this.houseBillOfLadingNumber = houseBillOfLadingNumber;
    }

    @Override
    public boolean equals(java.lang.Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        TrackingIdentifier trackingIdentifier = (TrackingIdentifier) o;
        return Objects.equals(this.id, trackingIdentifier.id)
                && Objects.equals(this.carrierTracking, trackingIdentifier.carrierTracking)
                && Objects.equals(this.acsin, trackingIdentifier.acsin)
                && Objects.equals(this.aftn, trackingIdentifier.aftn)
                && Objects.equals(this.containerNumber, trackingIdentifier.containerNumber)
                && Objects.equals(this.houseBillOfLadingNumber, trackingIdentifier.houseBillOfLadingNumber);
    }

    @Override
    public int hashCode() {
        return Objects.hash(id, carrierTracking, acsin, aftn, containerNumber, houseBillOfLadingNumber);
    }

    @Override
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("class TrackingIdentifier {\n");
        sb.append("    id: ").append(toIndentedString(id)).append("\n");
        sb.append("    carrierTracking: ")
                .append(toIndentedString(carrierTracking))
                .append("\n");
        sb.append("    acsin: ").append(toIndentedString(acsin)).append("\n");
        sb.append("    aftn: ").append(toIndentedString(aftn)).append("\n");
        sb.append("    containerNumber: ")
                .append(toIndentedString(containerNumber))
                .append("\n");
        sb.append("    houseBillOfLadingNumber: ")
                .append(toIndentedString(houseBillOfLadingNumber))
                .append("\n");
        sb.append("}");
        return sb.toString();
    }

    /** Convert the given object to string with each line indented by 4 spaces (except the first line). */
    private String toIndentedString(java.lang.Object o) {
        if (o == null) {
            return "null";
        }
        return o.toString().replace("\n", "\n    ");
    }
}
