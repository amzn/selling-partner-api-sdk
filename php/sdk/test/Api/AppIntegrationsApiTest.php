...
class AppIntegrationsApiTest extends TestCase
{
    ...
    public function testCreateNotification200()
    {
        try {
            ...
            if ($this->testHelper->shouldSkipTest('testCreateNotification200')) {
                $this->assertTrue(true);
                return;
            }

            //　Build Request Json for Request to static SandBox
            $invalidRequestJson = '{body&#x3D;{value&#x3D;{templateId&#x3D;PRICE_CHANGE, marketplaceId&#x3D;ATVPDKIKX0DER, notificationParameters&#x3D;{}}}};';
            ...
            $invalidResponseJson = '{notificationId&#x3D;0cf25616-f68c-4eba-a5d3-6823b61506c0}';
    ...
    public function testCreateNotification400()
    {
        try {
            // Skip test if it is in the skip list
            if ($this->testHelper->shouldSkipTest('testCreateNotification400')) {
                $this->assertTrue(true);
                return;
            }

            //　Build Request Json for Request to static SandBox
            $invalidRequestJson = '{body&#x3D;{value&#x3D;{templateId&#x3D;BAD_TEMPLATE_ID, marketplaceId&#x3D;ATVPDKIKX0DER, notificationParameters&#x3D;{}}}};';
            // Prepare request parameters
            $requestParams = $this->testHelper->prepareRequestParamsFromMethod(
                $this->apiInstance,
                'createNotification',
            ...
            //Build Expected Response Json for Assert
            $invalidResponseJson = '{errors&#x3D;[{code&#x3D;InvalidInput, message&#x3D;Invalid Template Identifier(templateId), details&#x3D;&#39;BAD_TEMPLATE_ID&#39; is an invalid templateId}]}';
            // Prepare expected response
            $expectedResponse = $this->testHelper->prepareExpectedResponse(
                $this->apiInstance,
                'createNotification',
    ...
    public function testDeleteNotifications204()
    {
        try {
            // Skip test if it is in the skip list
            if ($this->testHelper->shouldSkipTest('testDeleteNotifications204')) {
                $this->assertTrue(true);
                return;
            }

            //　Build Request Json for Request to static SandBox
            $invalidRequestJson = '{body&#x3D;{value&#x3D;{templateId&#x3D;PRICE_CHANGE, deletionReason&#x3D;INCORRECT_RECIPIENT}}};';
            // Prepare request parameters
            $requestParams = $this->testHelper->prepareRequestParamsFromMethod(
                $this->apiInstance,
                'deleteNotifications',
    ...
    public function testDeleteNotifications400()
    {
        try {
            // Skip test if it is in the skip list
            if ($this->testHelper->shouldSkipTest('testDeleteNotifications400')) {
                $this->assertTrue(true);
                return;
            }

            //　Build Request Json for Request to static SandBox
            $invalidRequestJson = '{body&#x3D;{value&#x3D;{templateId&#x3D;PRICE_CHANGE_INVALID, deletionReason&#x3D;INCORRECT_RECIPIENT}}};';
            // Prepare request parameters
            $requestParams = $this->testHelper->prepareRequestParamsFromMethod(
                $this->apiInstance,
                'deleteNotifications',
            ...
            //Build Expected Response Json for Assert
            $invalidResponseJson = '{errors&#x3D;[{code&#x3D;InvalidInput, message&#x3D;Invalid Template Id(templateId), details&#x3D;&#39;PRICE_CHANGE_INVALID&#39; is an invalid templateId}]}';
            // Prepare expected response
            $expectedResponse = $this->testHelper->prepareExpectedResponse(
                $this->apiInstance,
                'deleteNotifications',
    ...
    public function testRecordActionFeedback204()
    {
        try {
            // Skip test if it is in the skip list
            if ($this->testHelper->shouldSkipTest('testRecordActionFeedback204')) {
                $this->assertTrue(true);
                return;
            }

            //　Build Request Json for Request to static SandBox
            $invalidRequestJson = '{notificationId&#x3D;{value&#x3D;0cf25616-f68c-1234-a5d3-6823b61506c0}, body&#x3D;{value&#x3D;{feedbackActionCode&#x3D;SELLER_ACTION_COMPLETED}}};';
            // Prepare request parameters
            $requestParams = $this->testHelper->prepareRequestParamsFromMethod(
                $this->apiInstance,
    ...
    public function testRecordActionFeedback400()
    {
        try {
            // Skip test if it is in the skip list
            if ($this->testHelper->shouldSkipTest('testRecordActionFeedback400')) {
                $this->assertTrue(true);
                return;
            }

            //　Build Request Json for Request to static SandBox
            $invalidRequestJson = '{notificationId&#x3D;{value&#x3D;0cf25616-f68c-a5d3-6823b61506c0}, body&#x3D;{value&#x3D;{feedbackActionCode&#x3D;SELLER_ACTION_COMPLETED}}};';
            // Prepare request parameters
            $requestParams = $this->testHelper->prepareRequestParamsFromMethod(
                $this->apiInstance,
                'recordActionFeedback',
            ...
            //Build Expected Response Json for Assert
            $invalidResponseJson = '{errors&#x3D;[{code&#x3D;InvalidInput, message&#x3D;Invalid notificationId, details&#x3D;&#39;0cf25616-f68c-a5d3-6823b61506c0&#39; is an invalid notificationId}]}';
            // Prepare expected response
            $expectedResponse = $this->testHelper->prepareExpectedResponse(
                $this->apiInstance,
                'recordActionFeedback',
