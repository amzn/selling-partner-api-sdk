...
class Budget implements ModelInterface, \ArrayAccess, \JsonSerializable
{
     ...
     * Sets type.
     *
     * @param string $type the budget type, either monetary or unit-based
     */
     ...
     * Sets value.
     *
     * @param float $value budget value (amount or unit count)
     */
    public function setValue(float $value): self
    {
        ...
        if (!ObjectSerializer::getSkipModelValidation()) {
            if ($value <= 0) {
                throw new \InvalidArgumentException('invalid value for $value when calling Budget., must be bigger than 0.');
            }
        }
     ...
     * Sets currency_code.
     *
     * @param null|string $currency_code The currency code in ISO 4217 format. Required when `type` is `AMOUNT`.
     */
    public function setCurrencyCode(?string $currency_code): self
    {
        ...
        if (!ObjectSerializer::getSkipModelValidation()) {
            if (!is_null($currency_code) && (mb_strlen($currency_code) > 3)) {
                throw new \InvalidArgumentException('invalid length for $currency_code when calling Budget., must be smaller than or equal to 3.');
            }
            if (!is_null($currency_code) && (mb_strlen($currency_code) < 3)) {
                throw new \InvalidArgumentException('invalid length for $currency_code when calling Budget., must be bigger than or equal to 3.');
            }
        }
